...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void didChangeDependencies() async {
    ...
    _streamSubscription = _firestore
        ...
        .doc(args['conversationName'].toString())
        .collection('messages')
        .snapshots()
        .listen((event) {
      setState(() {});
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () async {
      var args =
          ModalRoute.of(context)?.settings.arguments as Map<String, String>?;
      Query<Map<String, dynamic>> collectionRef = await _firestore
          .collection('users')
          .doc((loggedInUser?.email).toString())
          .collection('conversations')
          .doc(args?['conversationName'].toString())
      ...
      querySnapshot.docs.forEach((doc) {
        if (doc.data() != null) {
          ...
          conversations
              .add({'role': data['sender'], 'content': data['message']});
        }
      });
    });
  }
    ...
    return ScaffoldMessenger(
      ...
      child: Scaffold(
          ...
          appBar: AppBar(
              ...
              elevation: 0,
              backgroundColor: foregroundColor,
              title: Text('${args['conversationName']}',
                        ...
                        child: StreamBuilder<QuerySnapshot>(
                            ...
                            builder: (context, snapshot) {
                              ...
                              for (var message in messages) {
                                final sender =
                                    (message.data() as dynamic)['sender'];
                                final text =
                                    (message.data() as dynamic)['message'];
                                final isImage =
                                    (message.data() as dynamic)['isImage'];
                                ...
                                if (!messageTextController.text.isEmpty) {
                                  ...
                                  if (message.isEmpty) {
                                    ...
                                    String generateImage =
                                        await gptService.getResponse([
                                      {
                                        'role': 'user',
                                        'content':
                                            'Does the message "$message" is asking to generate an image or an art or something similar? Simply reply me either yes or no'
                                      }
                                    ]);
                                    if (generateImage
                                        .toLowerCase()
                                        .contains('yes')) {
                                      String url = await dalleService
                                          .getResponse(message);
                                      await _firestore
                                          .collection('users')
                                          .doc((loggedInUser?.email).toString())
                                          .collection('conversations')
                                          .doc(args['conversationName']
                                              .toString())
                                          .collection('messages')
                                          .add({
                                        'sender': 'assistant',
                                      ...
                                      message = await gptService
                                          .getResponse(conversations);
                                      await _firestore
                                          .collection('users')
                                          .doc((loggedInUser?.email).toString())
                                          .collection('conversations')
